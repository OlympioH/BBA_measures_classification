...
from ht_learning import ecc_theorique, ht_theorique
...
def get_dgm_combined(dgms): # Dgms must be ordered as (Ord0, Rel1, Ext0+, Ext1-)
    dgm_combined = []
    max_val = max([np.max(dgm) for dgm in dgms])
    for k in range(4):
        dgm = dgms[k]
        for i in range(dgm.shape[0]):
            ...
            if k == 0 : # Ord0
                dim, a, b = 0, c, d
            if k == 1 : # Rel1
                dim, a, b = 1, 2*max_val - d, 2*max_val - c
            if k == 2 : # Ext0+
                dim, a, b = 0, c, 2*max_val - d
            if k == 3 : # Ext1-
                dim, a, b = 1, d, 2*max_val - c 
        dgm_combined.append((dim, (a,b)))
    return dgm_combined

def ecc_graph(dgms, t_min, t_max, resolution): # Dgms must be ordered as (Ord0, Rel1, Ext0+, Ext1-)
    dgm_combined = get_dgm_combined(dgms)
    euler_curve = ecc_theorique(dgm_combined, t_min, t_max, resolution)
    return euler_curve

def ht_graph(dgms, kernel): # Dgms must be ordered as (Ord0, Rel1, Ext0+, Ext1-)
    dgm_combined = get_dgm_combined(dgms)
    ht_fun, dgm_np = ht_theorique(dgm_combined, kernel, None)
    return ht_fun
